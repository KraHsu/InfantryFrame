...
extern void elog_port_output(uint8_t t_id, const char *log, size_t size);
...
void elog_start(void) {
    ...
    /* show version */
    logInfo("EasyLogger V%s is initialize success.\n", ELOG_SW_VERSION);
}
...
void elog_stop(void) {
    ...
    /* show version */
    logInfo("EasyLogger V%s is deinitialize success.\n", ELOG_SW_VERSION);
}
...
#else
    elog_port_output(0, log_buf, log_len);
#endif
    /* unlock output */
    elog_output_unlock();
...
void elog_output(
    ...
    if (get_fmt_enabled(level, ELOG_FMT_DIR | ELOG_FMT_FUNC | ELOG_FMT_LINE)) {
        ...
        /* package func info */
        if (get_fmt_enabled(level, ELOG_FMT_FUNC) && !(strlen(func) == 10 && func[8] != '(')) {
    ...
    elog_buf_output(log_buf, log_len);
#else
    elog_port_output(t_id, log_buf, log_len);
#endif
    /* unlock output */
    elog_output_unlock();
}
